...
and break_token_column
  ...
  let rec sub buf1 buf1_after_spaces buf2 buf2_after_spaces lst =
    ...
    | (rule_with_comment, config)::xs -> (
      ...
      in
      if List.length str_lst > 1 || is_last_exists_after_comment then
        let () =
        ...
        if buf1_len + buf1_after_spaces + buf2_len + buf2_after_spaces + code_str_len <= (len_max ctx) then
          ...
          | Some(false) -> (
            (* 改行不可ポイント *)
            if is_string_empty buf1 then
              sub code_str (config.space_size |> option_unwrap_or 1) "" 1 xs
            else
          ...
          | None -> (
            ...
            *)
            ...
            in
            sub buf1 (config.space_size |> option_unwrap_or 1) "" 1 xs
          )
        else
          (* 複数に改行しなければならない *)
          if buf2_len == 0 then
